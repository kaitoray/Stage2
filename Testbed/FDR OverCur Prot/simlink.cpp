...
static void change(uint64_t ts)
{
    ...
    second = secs % SEC_PER_MIN;
     ...
     //Open File//
    	fp=fopen("/media/sf_sharing/Simulation testbed for Stage 1/Datasets/Simulink/IED_FDR_physical.csv","a");
...
int main()
{
    ...
    //Open File//
    	fp=fopen("/media/sf_sharing/Simulation testbed for Stage 1/Datasets/Simulink/IED_FDR_physical.csv","w");
    	if(fp==NULL) {
        printf("File cannot open! " );
        exit(0);
     	}
    //write values to file//
    	fprintf(fp,"year,month,day,hour,minute,second,millisecond,UTC,open_CB1-22KV,open_CB2-22KV,open_CB3-22KV,open_CB1-FDR1,open_CB2-FDR2,open_CB-FDR3,open_CB-FDR4,I_FDR1,I_FDR2,I_FDR3,I_FDR4\n");
    //Close file//
    	fclose(fp);
...
//		printf(" FDR1: %d A\n FDR2: %d A\n FDR3: %d A\n FDR4: %d A\n Reset all CB: %d\n Trip CB-FDR1: %d\n Trip CB-FDR2: %d\n Trip CB-FDR3: %d\n Trip CB-FDR4: %d\n Trip CB1-22KV: %d\n Trip CB3-22KV: %d\n\n", stations_data[0].analogIn[0], stations_data[0].analogIn[1], stations_data[0].analogIn[2], stations_data[0].analogIn[3], stations_data[0].digitalIn[0], stations_data[0].digitalOut[0], stations_data[0].digitalOut[1], stations_data[0].digitalOut[2], stations_data[0].digitalOut[3], stations_data[0].digitalOut[4], stations_data[0].digitalOut[5]);
	...
	FILE *fp;
    	...
    	CBstval [3] = stations_data[0].digitalOut[0];
    	CBstval [4] = stations_data[0].digitalOut[1];
    	CBstval [5] = stations_data[0].digitalOut[2];
    	CBstval [6] = stations_data[0].digitalOut[3];
    	CBstval [0] = stations_data[0].digitalOut[4];
    	CBstval [2] = stations_data[0].digitalOut[5];
