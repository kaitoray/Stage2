...
/*
//Open File//
    	fp=fopen("/media/sf_sharing/Simulation testbed for Stage 1/Datasets/CB_status/TRSF2.csv","a");
	...
	fprintf(fp,"%ld,%d,%d,%d,%d,%d,%d,", ts-36000, year, month, day, hour, minute, second);
    //Close file//
    	fclose(fp);
*/    	
...
{
    printf("GOOSE event: PIOC_TRSF2_CBStval\n");
...
//############### store CBstval to local for control signal ##########################//  
    //Open File//
    	fp=fopen("/home/ray/Desktop/Trsf1 OverCur Prot/TripRecord/TripFromTRSF2.csv","w");
...
//###############store CBstval to share as datasets##########################//     	
    ...
    //write values to file//
    	fprintf(fp,"%u,", (uint32_t) (timestamp % 1000));
	for (i=0; i<4; i++) {
	fprintf(fp,"%d,", CBstval[i]);
	}
...
/*    
    //Open File//
    	fp=fopen("/media/sf_sharing/Simulation testbed for Stage 1/Datasets/CB_status/TRSF2.csv","w");
    ...
    //write values to file//
	fprintf(fp,"year,month,day,hour,minute,second,millisecond,UTC,CB2-66KV,CB3-66KV,CB-TRSF2,stNum,sqNum\n");
    //Close file//
    	fclose(fp);
*/    		
    GooseSubscriber subscriber = GooseSubscriber_create("SimulationTestbed/PIOC$TRSF2$CBStval", NULL);

    GooseSubscriber_setAppId(subscriber, 0x8002);
