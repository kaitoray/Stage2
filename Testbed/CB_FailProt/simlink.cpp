...
static void change(uint64_t ts)
{
    ...
    second = secs % SEC_PER_MIN;
     ...
     //Open File//
    	fp=fopen("/media/sf_sharing/Simulation testbed for Stage 1/Datasets/Simulink/IED_CB_FailProt_physical.csv","a");
...
int main()
{
    ...
    //Open File//
    	fp=fopen("/media/sf_sharing/Simulation testbed for Stage 1/Datasets/Simulink/IED_CB_FailProt_physical.csv","w");
    	if(fp==NULL) {
        printf("File cannot open! " );
        exit(0);
     	}
    //write values to file//
    	fprintf(fp,"year,month,day,hour,minute,second,millisecond,UTC,open_CB-TRSF1,open_CB-TRSF2,open_CB1-22KV,open_CB2-22KV,open_CB3-22KV,Status_CB1-22KV,Status_CB3-22KV,Status_CB-FDR1,Status_CB-FDR4\n");
    //Close file//
    	fclose(fp);
...
//#################store physical parameters with timestamp#########################//
   ...
   // convert now to string form
	...
	for (i=0; i<4; i++) {
	fprintf(fp,"%d,", stations_data[0].digitalIn[i+1]);
	}
