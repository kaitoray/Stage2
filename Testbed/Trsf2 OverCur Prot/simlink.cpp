...
static void change(uint64_t ts)
{
    ...
    second = secs % SEC_PER_MIN;
     ...
     //Open File//
    	fp=fopen("/media/sf_sharing/Simulation testbed for Stage 1/Datasets/Simulink/IED_TRSF2_physical.csv","a");
...
int main()
{
    ...
    //Open File//
    	fp=fopen("/media/sf_sharing/Simulation testbed for Stage 1/Datasets/Simulink/IED_TRSF2_physical.csv","w");
    	if(fp==NULL) {
        printf("File cannot open! " );
        exit(0);
     	}
    //write values to file//
    	fprintf(fp,"year,month,day,hour,minute,second,millisecond,UTC,open_CB2-66KV,open_CB3-66KV,open_CB-TRSF2,open_CB2-22KV,I_CB2-66KV,I_CB3-66KV,I_TRSF2-W1,I_TRSF2-W2,I_CB-TRSF2,I_CB2-22KV\n");
    //Close file//
    	fclose(fp);
