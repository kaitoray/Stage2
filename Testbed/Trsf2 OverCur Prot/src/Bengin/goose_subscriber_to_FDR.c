...
/*     
    //Open File//
    	fp=fopen("/media/sf_sharing/Simulation testbed for Stage 1/Datasets/CB_status/FDR.csv","a");
...
{
    printf("GOOSE event: PIOC_FDR_CBStval\n");
...
//############### store CBstval to local for control signal ##########################//  
    //Open File//
    	fp=fopen("/home/ray/Desktop/Trsf2 OverCur Prot/TripRecord/TripFromFDR.csv","w");
...
/*    
    //Open File//
    	fp=fopen("/media/sf_sharing/Simulation testbed for Stage 1/Datasets/CB_status/FDR.csv","w");
    ...
    //write values to file//
	fprintf(fp,"year,month,day,hour,minute,second,millisecond,UTC,CB1-22KV,CB2-22KV,CB3-22KV,CB-FDR1,CB-FDR2,CB-FDR3,CB-FDR4,stNum,sqNum\n");
    //Close file//
    	fclose(fp);
*/
    		
    GooseSubscriber subscriber = GooseSubscriber_create("SimulationTestbed/PIOC$FDR$CBStval", NULL);

    GooseSubscriber_setAppId(subscriber, 0x8003);
